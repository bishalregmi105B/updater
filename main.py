...
__copyright__ = 'Copyright (C) 2020, Ashlya Tech'
__version__ = '1.0'
        ...
        def check_updates():
            try:
                ...
                response = requests.get(
                    'https://brightstarpublicschool.edu.np/bspssoftware/bspslatestversion.txt')
                data = response.text
...
class UpdateManager(tk.Toplevel):
    def __init__(self, parent):
        ...
        def install_update():
            win32api.ShellExecute(0, 'open', f'update\\{_AppName_}.msi', None, None, 10)
            parent.destroy()
        ...
        def start_update_manager():
            with requests.get('https://github.com/vsantiago113/Tkinter-MyTestApp/raw/master/'
                ...
                r.raise_for_status()
                with open(f'./update/Ashlya.msi', 'wb') as f:
...
class DisplayAboutMe(tk.Toplevel):
    def __init__(self, parent):
        ...
        def call_link_youtube(*args):
            webbrowser.open_new_tab('https://www.youtube.com/channel/UCorN_YWKe6CsBVx6b9ObZIw')

        tk.Label(f1, text=_AppName_ + ' ' + str(__version__)).pack()
        tk.Label(f1, text='Copyright (C) Ashlya Tech').pack()
        ...
        labe_2.pack()
        labe_2.bind('<Button-1>', call_link_youtube)
